...
Original file is located at
    https://colab.research.google.com/drive/1AZCT6LU-XdzG8VUYbZLNMx-qsVENNIAq

# Project 5: Vision (Part 2)

Due Date: Monday, March 23, 2020 @ 11:59 P.M.

Student #1 Name: Spencer Shook

Student #2 Name: Rushabh Varia

Student #3 Name: Divij Sachdeva

Student #4 Name: Vijay Nambi
...
class LaneFinder:
  ...
  def compute_orientation(self, image):
    ...
    #########################################################################
    #By Divij Sachdeva
    Z_in_camera_frame_list = [z/np.cos(self.camera_pitch) for z in self.scan_lines]
  ...
  def compute_v_coordinate_of_scan_line_in_image(self, Z):
    ...
    # ground.
    Z_in_road_frame = Z * np.cos(self.camera_pitch)
    Y = self.height - np.tan(self.camera_pitch) * Z_in_road_frame
  ...
  def find_lane_boundaries_on_scan_line(self, image, v):
    ...
    mid = 0
    print(newInd)
    for i in range(len(newInd) - 1):
      if newInd[i + 1] - newInd[i] > 1:
        mid = i + 1
        break

    lArr = newInd[0:mid]
    rArr = newInd[mid:len(newInd)]

    #print(lArr)
    #print(rArr)

    u_left = (lArr[0] + lArr[len(lArr) - 1]) / 2
    u_right = (rArr[0] + rArr[len(rArr) - 1]) / 2
  ...
  def compute_camera_coordinates_from_image_coordinates(self, u, v, Z):
    ...
    Y = self.height - np.tan(self.camera_pitch) * Z * np.cos(self.camera_pitch)
    #########################################################################
    #                             END OF YOUR CODE                          
    #########################################################################
  ...
  def compute_line_direction_from_points(self, points):
    ...
    # Spencer
    line = np.array([points[1][0] - points[0][0], points[1][1] - points[0][1], points[1][2] - points[0][2]])
  ...
  def compute_angle_of_lane_with_optical_axis(self, lane_line):
    ...
    # Spencer - I'm pretty sure the Z axis vector should be (0, 0, 1), but I'm not positive
...
#########################################################################
image_path = '/content/drive/My Drive/NewLaneImgs/'
#########################################################################
#                             END OF YOUR CODE                          
#########################################################################
