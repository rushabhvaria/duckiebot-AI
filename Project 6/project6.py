...
Original file is located at
    https://colab.research.google.com/drive/1dPtu8U8XBQhsWpd2-ZbUeUpJwkw2_-M_
...
def transform_cloud(bTa, clouda):
    ...
    shape = clouda.shape[1]

    tempCloud = np.append(clouda,np.ones((1,shape)),axis=0)

    cloudTrans = np.matmul(bTa.matrix(), tempCloud)

    cloudTrans = np.multiply(cloudTrans,cloudTrans[3,:])
...
def assign_closest_pairs(clouda, cloudb):
    ...
    neigh = NearestNeighbors(n_neighbors=1).fit(dst.T)
    indices = neigh.kneighbors(src.T, return_distance=False)
    return dst[:,np.ravel(indices)]
